r"""CIFAR-100 BACON graded-logic CBM runner (LogicCBM-comparable).

Same two-stage pipeline as ``run_awa`` with (K=925 concepts, 100 classes):
  joint : InceptionV3 -> 925 concept logits -> 100-class linear, end-to-end,
          CE(class) + attr_lam * imbalance-weighted BCE(concept vs the class-level
          925 MuCIL/LogicCBM concepts). Saves an ``--init-from``-compatible ckpt.
  tree  : warm-start the joint concepts and train our recttree graded-logic head
          (100 trees over 925 concepts) via ``cub_emergent.train_one``.

    py -3 run_cifar.py joint --epochs 60 --batch-size 64
    py -3 run_cifar.py tree  --init-from saved/cifar_joint_inception_v3_k925.pt \
...
def _eval_joint(model, loader, device):
    model.eval()
    correct = total = c_correct = c_total = 0
...
def run_joint(args):
    ...
    tl = DataLoader(cifar_data._CIFARImages("train", True, image_size=image_size),
                    batch_size=args.batch_size, shuffle=True, num_workers=args.workers,
                    pin_memory=True, drop_last=True)
    vl = DataLoader(cifar_data._CIFARImages("test", False, image_size=image_size),
                    batch_size=args.batch_size, shuffle=False, num_workers=args.workers,
                    pin_memory=True)
    model = CifarJoint(args.backbone).to(device)
    pos_weight = cifar_data.find_imbalance().to(device)
    ...
    for ep in range(args.epochs):
        ...
        for img, c, y in tl:
            img, y, c = img.to(device), y.to(device), c.to(device)
            yl, cl = model(img)
            # LogicCBM-faithful: SUM per-attribute BCE + normalize by (1+attr_lam*K).
            con_loss = F.binary_cross_entropy_with_logits(
                cl, c, pos_weight=pos_weight, reduction="none").sum(dim=1).mean()
            loss = (F.cross_entropy(yl, y) + args.attr_lam * con_loss) / (1.0 + args.attr_lam * _K)
            opt.zero_grad()
            loss.backward()
            opt.step()
            tot += float(loss)
...
def run_tree(args):
    ...
    vl = DataLoader(cifar_data._CIFARImages("test", False, image_size=image_size),
                    batch_size=args.batch_size, shuffle=False, num_workers=args.workers,
                    pin_memory=True)
    pos_weight = cifar_data.find_imbalance()
    acc, model = train_one(
        _K, tl, vl, device, args.epochs, seed=args.seed, head="recttree", n_species=_N_CLASSES,
    ...
    tag = f"cifar_recttree_w{args.rect_width}_l0{args.edge_l0:g}{st}_s{args.seed}{ut}"
    ...
    torch.save({"state_dict": model.state_dict(), "K": _K, "n_classes": _N_CLASSES,
                ...
                "rect_width": args.rect_width, "rect_depth": args.rect_depth,
                "rect_layer_widths": None, "rect_max_parents": args.max_parents,
...
def main():
    ...
    j.add_argument("--batch-size", type=int, default=64)
    ...
    j.add_argument("--out", default=os.path.join(_HERE, "saved", "cifar_joint_inception_v3_k925.pt"))
    ...
    args = ap.parse_args()
    run_joint(args) if args.stage == "joint" else run_tree(args)
