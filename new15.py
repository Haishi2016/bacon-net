...
def evolve_trees(variables, dataset, generations=500, population_size=100, mutation_rate=0.2, grow_chance=0.03):
    """Evolve trees using mixed mutation and track improvements dynamically."""
    population = generate_constrained_tree(variables)  # Start with small trees
    best_fitness = float('-inf')  # Track best fitness
    best_tree = None
    ...
    for gen in range(generations):
        ...
        # **Mutation: Mix 'a' tuning with tree expansion**
        while len(new_population) < population_size:
            parent = random.choice(survivors)
            child = mixed_mutation(parent, mutation_rate, grow_chance)
            new_population.append(child)

        population = new_population  # Update population for next generation

    return best_tree  # Return best evolved tree
