...
class InputToLeafSinkhorn(nn.Module):
    def __init__(self, num_inputs, num_leaves, temperature=0.5, sinkhorn_iters=20):
...
def generate_data(repeat_factor=100):
    ...
    for _ in range(repeat_factor):
        for a, b, c, d in base_cases:
            y = a and (b or c) and d # Compute function
...
def train_and_select_best_model(weight_mode="trainable", weight_value=1.0, weight_range=(0.5, 2.0), weight_choices=None):
    ...
    model = train_model(model, X_train, Y_train, epochs=12000)
...
def print_estimated_expression(model):
    with torch.no_grad():
        perm_matrix = sinkhorn(model.input_to_leaf.logits, n_iters=30, temperature=0.05).numpy()
...
def extract_hard_permutation(model):
    with torch.no_grad():
        perm_matrix = sinkhorn(model.input_to_leaf.logits, n_iters=50, temperature=0.05)
        assignment = perm_matrix.argmax(dim=1)  # Which input each leaf picks
        return assignment
...
if __name__ == "__main__":
    ...
    # 🔍 Extract hard input-leaf mapping
