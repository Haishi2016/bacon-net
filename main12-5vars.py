...
def sample_topk_permutations(num_inputs, k, model_template, X, Y, weight_mode, weight_value, weight_range, weight_choices):
    ...
    losses = []
    print(f"🧠 Evaluating {len(all_perms)} permutations...")
...
def generate_data(repeat_factor=100):
    data = []
    labels = []
    base_cases = list(itertools.product([0, 1], repeat=5))  # Truth table

    for _ in range(repeat_factor):
        for a, b, c, d, e in base_cases:
            y = (((a and b) or c) or d) and e # Compute function
