...
def gcd_operator(a, x, y):
    """Generalized GCD function with andness parameter."""
    return a * min(x, y) + (1 - a) * max(x, y)

# Variable constraints (each variable must appear within this range)
VARIABLE_RANGES = {"x1": (1, 1), "x2": (1, 1)}  # Each variable must appear at least once, at most twice
DEPTH_PENALTY = 0.02  # Slight penalty for deeper trees
...
# **Prevent `IndexError` during tree generation**
def generate_constrained_tree(variables, required_counts=None, depth=0, max_depth=3):
    ...
    # If max depth is reached or no variables are left, select a valid variable
    remaining_vars = [v for v, count in required_counts.items() if count > 0]
    if not remaining_vars or depth >= max_depth:
        if remaining_vars:
            var = random.choice(remaining_vars)
            required_counts[var] -= 1
            return var
        else:
            return random.choice(list(VARIABLE_RANGES.keys()))  # Ensure valid output
    ...
    if not splittable_vars:
        splittable_vars = remaining_vars  # Fallback to remaining variables
    ...
    # **Ensure right subtree is different from left**
    while True:
...
# **Mutation now ensures valid trees**
def mutate_tree(tree, mutation_rate=0.2):
    """Mutate a tree while keeping variables valid and avoiding gcd(x, x)."""
    if isinstance(tree, str):  # Leaf node
        return tree

    a, left, right = tree

    # Mutate 'a' slightly, allowing full range
    ...
    # **Ensure no gcd(x, x) after mutation**
    if left == right:
        return left  # Replace gcd(a, x, x) → x

    return (a, mutate_tree(left, mutation_rate), mutate_tree(right, mutation_rate))

# Fitness function strongly penalizes incorrect variable appearances
def fitness(tree, dataset):
    """Compute fitness with depth penalty and variable usage enforcement."""
    errors = []
    ...
    return -(mse + depth_penalty + variable_penalty)  # Negative because GA maximizes fitness
