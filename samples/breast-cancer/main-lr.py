"""Logistic Regression baseline for Breast Cancer Wisconsin Dataset"""
import numpy as np
...
def main():
    ...
    print("="*80)
    print("LOGISTIC REGRESSION BASELINE - BREAST CANCER WISCONSIN")
    print("="*80)
    ...
    for solver in solvers:
        ...
        results.append((solver, test_acc, test_f1, test_auprc))
    
    # Summary
    print("\n" + "="*80)
    print("RESULTS SUMMARY")
    print("="*80)
    print(f"{'Solver':<15} {'Test Accuracy':<15} {'Test F1':<15} {'Test AUPRC':<15}")
    print("-" * 80)
    for solver, acc, f1, auprc in results:
        print(f"{solver:<15} {acc:<15.4f} {f1:<15.4f} {auprc:<15.4f}")
    
    # Best model
    best_solver, best_acc, best_f1, best_auprc = max(results, key=lambda x: x[2])
    print(f"\nBest Model: {best_solver} (F1: {best_f1:.4f}, AUPRC: {best_auprc:.4f})")
