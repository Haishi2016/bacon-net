...
x1, y1 = dataCreator.create(
    500, 1, lambda x: x[0] + x[1] + 2 * x[2], params=3)
stack1.fit2(x1, y1, maeTarget=0.001, verbose=True, weightShift='random')
m1 = stack1.explain()
...
x2, y2 = dataCreator.create(
    1000, 1, lambda x: np.maximum(np.minimum(np.maximum(np.minimum(x[0], x[1]), x[2]), x[3]), x[4]), params=paramCount)
history, mae, counter = stack2.fit2(
    x2, y2, maeTarget=0.001, verbose=True, weightShift='random')
