...
class BaconAdditionModel(nn.Module):
    ...
    def forward_single_sum(self, x1, x2, tau: float, sum_k: int, hard: bool = False):
        ...
        if self.single_sum is None:
            # Use the k-th BACON head from the multi-head set
...
def main():
    ...
    ap.add_argument("--epochs", default=1200, type=int)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # How many (i,j) pairs give each sum k?
        pair_counts = [min(k + 1, 19 - k) for k in range(num_sums)]
        pair_counts = np.array(pair_counts, dtype=np.float32)
        pair_probs = pair_counts / pair_counts.sum()

        # Pre-sample which sum to train each epoch
        # (you can also do simple round-robin if you prefer)
        epoch_sums = np.random.choice(
            np.arange(num_sums), size=args.epochs, p=pair_probs
        )
        ...
        # pick active sum for this epoch
        active_sum = int(epoch_sums[epoch - 1]) 
