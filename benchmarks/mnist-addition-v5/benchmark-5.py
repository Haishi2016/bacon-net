...
class BaconAdditionModel(nn.Module):
    ...
    Modes:
      - multi-sum (single_sum is None):
          19 BACON trees, one per sum 0..18 => 19-way output.
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor, tau: float, hard: bool = False):
        """
        Multi-sum forward:
          - If single_sum is None: returns [B,19] with all heads.
          - If single_sum is K: returns [B] for that K.
        """
        concepts = self._concepts_from_images(x1, x2, tau, hard)

        
        y = self.bacon(concepts)   # [B] or [B,1]
        y_prob = y.view(-1)               # [B]
...
def main():
    ...
    ap.add_argument("--single-sum", type=int, default=None,
                    help="If set (0..18), train a single BACON head for 'sum==K'. "
                         "If None, we train 19 heads but only one active per epoch.")

    ap.add_argument(
        "--epochs-per-sum",
        default=5,
        type=int,
        help="Number of consecutive epochs to train on the same active sum (multi-sum only).",
    )

    # Partial/noisy supervision (still used only for reporting in eval)
    ap.add_argument(
        "--supervised-sums",
        type=str,
        default=None,
        help="Comma-separated list of sums (0-18) considered 'supervised' for reporting."
    )
    ap.add_argument(
        "--unsup-mode",
        type=str,
        ...
        help="Kept for compatibility but not used in the new per-head training scheme."
    )
    ap.add_argument(
        "--lambda-consistency",
        default=0.1,
    ...
    # Parse supervised sums (used only for separated accuracy reporting)
    if args.single_sum is not None and args.supervised_sums is not None:
        ...
        if args.supervised_sums is not None:
            ...
            print(f"Using supervised SUM labels (for reporting): {supervised_sums}")
        else:
            supervised_sums = None
        supervised_sums_tensor = None
    ...
    # Pre-sample which sum head to train each epoch (multi-sum mode)
    num_sums = 19
    if args.single_sum is None:
        pair_counts = np.array([min(k + 1, 19 - k) for k in range(num_sums)],
                               dtype=np.float32)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.single_sum is None:
             # pick active sum for this epoch based on block index
            ...
            for (x1, x2), y in train_loader:
                ...
                # Forward through only the active head
                y_prob_k, c_prob = model.forward_single_sum(
                    x1, x2, tau=tau, sum_k=active_sum, hard=False
                )  # [B], [B,20]

                y_bin = (y == active_sum).float()       # [B]
                p = y_prob_k.clamp(1e-6, 1 - 1e-6)      # [B]
                log_p = torch.log(p)
                ...
                if args.lambda_consistency > 0.0:
                    # Consistency for the same active head
                    y_ab, _ = model.forward_single_sum(x1, x2, tau, sum_k=active_sum, hard=False)
                    y_ba, _ = model.forward_single_sum(x2, x1, tau, sum_k=active_sum, hard=False)
            ...
            # Single-sum training (sum==K? binary classification)
            K = args.single_sum

            for (x1, x2), y in train_loader:
                x1, x2, y = x1.to(device), x2.to(device), y.to(device)

                y_prob, c_prob = model(x1, x2, tau=tau, hard=False)  # [B]
                y_true = y.clone()
                ...
                y_ab, _ = model(x1, x2, tau, hard=False)
                y_ba, _ = model(x2, x1, tau, hard=False)
                loss_consistency = F.mse_loss(y_ab, y_ba)
        ...
        with torch.no_grad():
            ...
            for (x1, x2), y in test_loader:
                ...
                if args.single_sum is not None:
                    K = args.single_sum
                    y_prob_eval, _ = model(x1, x2, tau=args.tau_end, hard=True)  # [B]
                    y_bin = (y == K)
                    pred_bin = (y_prob_eval >= 0.5)
                    correct_all += (pred_bin == y_bin).sum().item()
                    tot_all += y.size(0)
                else:
                    # Build [B,19] by evaluating each head k
                    y_heads = []
                    for k in range(19):
                        yk, _ = model.forward_single_sum(
                            x1, x2, tau=args.tau_end, sum_k=k, hard=True
                        )  # [B]
                        y_heads.append(yk)
                    y_prob_eval = torch.stack(y_heads, dim=1)  # [B,19]
