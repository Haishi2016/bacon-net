...
from bacon.multiHeadBinaryTreeLogicNet import multiHeadBinaryTreeLogicNet
...
# ============================================================
# BACON-based Reasoning Model (100D input)
# ============================================================

class BaconAdditionModel(nn.Module):
    """
    Shared CNN + concept head:

        img1, img2
          ↓      ↓
        z1, z2 (128D each)
          ↓      ↓
        p1, p2 ∈ ℝ^{10}  (digit distributions)

    For BACON:
      - Build 100D feature:
            P_pair[i,j] = p1[i] * p2[j], flattened to [100]
      - Feed this 100D vector into BACON trees (paired structure).

    Modes:
      - multi-sum (single_sum is None):
          multi-head BACON: 19 outputs for sums 0..18.
      - single-sum (single_sum = K):
          one BACON tree for "sum == K?" with scalar output.
    """
    ...
    def __init__(
        ...
        if self.single_sum is None:
            # Multi-head: 19 BACON heads, all with 100D input, paired tree layout
            self.bacons = multiHeadBinaryTreeLogicNet(
                input_size=100,
                num_heads=19,
                is_frozen=False,
                weight_mode="fixed",
            ...
            # Single BACON tree for sum == K, 100D input
    ...
    def _build_features(self, x1, x2, tau: float, hard: bool):
        """
        Shared feature pipeline:
          - CNN → p1, p2 (10D each)
          - 20D concept vector for entropy/viz
          - 100D pairwise feature for BACON
        """
        # shared CNN
        ...
        # 20D concept distribution (for entropy regularizer + visualization)
        concepts_20 = torch.cat([p1, p2], dim=1)  # [B,20]

        # 100D pairwise features for BACON: outer product flattened
        # pair[i,j] = p1[i] * p2[j]
        outer = p1.unsqueeze(2) * p2.unsqueeze(1)    # [B,10,10]
        bacon_input = outer.view(outer.size(0), -1)  # [B,100]

        return bacon_input, concepts_20

    def forward(self, x1: torch.Tensor, x2: torch.Tensor, tau: float, hard: bool = False):
        bacon_input, concepts_20 = self._build_features(x1, x2, tau, hard)

        if self.single_sum is None:
            # Multi-sum mode: 19 outputs
            y_prob = self.bacons(bacon_input)  # [B,19]
        else:
            # Single-sum mode: one scalar per sample (prob sum==K)
            yk = self.bacon_single(bacon_input)  # [B,1] or [B]
            y_prob = yk.view(-1)                 # [B]

        c_prob = concepts_20  # [B,20] for entropy regularizer
    ...
    if model.single_sum is None:
        ...
        with torch.no_grad():
            for (x1, x2), _ in test_loader:
                ...
                # hard concepts for viz (independent of 100D BACON input)
                z1 = model.cnn(x1)
                z2 = model.cnn(x2)
...
def main():
    ...
    if args.save_path is not None:
        ...
        if args.single_sum is None:
            base_save_path = "best_bacon_mnist_addition_multiclass_100d.pt"
        else:
            base_save_path = f"best_bacon_mnist_addition_single_sum{args.single_sum}_100d.pt"
