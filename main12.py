...
def train_and_select_best_model(weight_mode="trainable", weight_value=1.0,
    ...
    def create_model():
        return BinaryTreeLogicNet(X_train.shape[1], weight_mode, weight_value, weight_range, weight_choices)
    ...
    for attempt in range(max_retries):
        ...
        for epoch in range(12000):
            ...
            if not frozen and loss.item() < freeze_loss_threshold:
                print(f"🧊 Freezing at epoch {epoch}, loss: {loss.item():.4f}")
                hard_perm = extract_hard_permutation(model)

                if is_repeat_permutation(hard_perm):
                    print("🔁 Permutation already seen. Restarting with perturbation.")
                    break

                mark_permutation_seen(hard_perm)
                model.input_to_leaf = FrozenInputToLeaf(hard_perm, model.original_input_size)
                optimizer = optim.Adam(model.parameters(), lr=0.01, weight_decay=1e-5)
                frozen = True
...
if __name__ == "__main__":
    model, loss = train_and_select_best_model(weight_mode="fixed", weight_value=1.0, max_retries=100)
