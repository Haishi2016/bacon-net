...
class BinaryTreeLogicNet(nn.Module):
    ...
    def print_tree_structure(self):
        """ Prints the binary tree structure with andness values (a). """
        ...
        for i in range(self.num_layers):
            ...
            # Store the new parent node in the next layer
            next_layer.append(parent)

        # Recursively format the tree structure
        def format_tree(node, depth=0):
            if isinstance(node, str) and node.startswith("Leaf"):
                return f"{'  ' * depth}{node}"
...
def train_and_select_best_model(weight_mode="trainable", weight_value=1.0, weight_range=(0.5, 2.0), weight_choices=None):
    ...
    model = train_model(model, X_train, Y_train, epochs=20000)
