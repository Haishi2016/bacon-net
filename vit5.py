...
class ViTMNISTClassifier:
    ...
    def collect_patch_metadata(self, num_images=200):
        self.model.eval()
        raw_patch_data = []
            ...
            for batch_idx, (images, labels) in enumerate(self.test_loader):
                ...
                for i in range(B):
                    for p in range(x.shape[1]):
                        patch = images[i].cpu().numpy().transpose(1, 2, 0)
                        y, x_ = divmod(p, 4)
                        patch_crop = patch[y*8:(y+1)*8, x_*8:(x_+1)*8, :]
                        patch_crop_gray = patch_crop.mean(axis=2) if patch_crop.shape[2] == 3 else patch_crop.squeeze()

                        if patch_crop_gray.var() < 0.01:
                            continue

                        raw_patch_data.append({
                            "embedding": x[i, p],
                            "image": patch_crop,
                            "digit": int(labels[i]),
                            "image_index": int(count + i),
                            "locations": [int(x_), int(y)]
                        })
                count += B
                if count >= num_images:
                    break

        print(f"📦 Merging {len(raw_patch_data)} raw patches...")
        merged_metadata = []
        for patch in raw_patch_data:
            found = False
            for entry in merged_metadata:
                if np.allclose(patch["embedding"], entry["embedding"], atol=1e-4):
                    entry["digit_labels"].add(patch["digit"])
                    entry["image_indices"].add(patch["image_index"])
                    entry["locations"].add(tuple(patch["locations"]))
                    found = True
                    break
            if not found:
                merged_metadata.append({
                    ...
                    "image": patch["image"],
                    "digit_labels": {patch["digit"]},
                    "image_indices": {patch["image_index"]},
                    "locations": {tuple(patch["locations"])}
                })

        print(f"✅ Reduced to {len(merged_metadata)} merged patches")
        return merged_metadata

    def save_patch_metadata(self, patch_metadata, filename="patch_metadata.json"):
        print(f"💾 Saving metadata to {filename}...")
        serializable = []
        for entry in patch_metadata:
            serializable.append({
                "digit_labels": sorted(list(entry["digit_labels"])),
                "image_indices": sorted(list(entry["image_indices"])),
                "locations": [list(xy) for xy in sorted(entry["locations"])]
            })
        with open(filename, "w") as f:
            json.dump(serializable, f, indent=2)
        print(f"✅ Metadata saved with {len(serializable)} entries.")

    def plot_patch_clusters(self, patch_metadata, num_clusters=10):
        print("📦 Extracting embeddings from metadata...")
        patch_embeddings = [entry["embedding"] for entry in patch_metadata]
        ...
        for cluster_id in range(num_clusters):
            ...
            for j, idx in enumerate(selected):
                patch = np.array(patch_metadata[idx]["image"])  # stored image
            ...
            if selected.size > 0:
                sample_meta = patch_metadata[selected[0]]
                axs[cluster_id, 0].set_ylabel(
                    f"Cluster {cluster_id}\nDigits: {sample_meta['digit_labels']}\nCoords: {sample_meta['locations']}",
                    fontsize=9
                )
...
if __name__ == "__main__":
    ...
    patch_metadata = classifier.collect_patch_metadata(num_images=200)
    classifier.save_patch_metadata(patch_metadata, filename="patch_metadata.json")
    classifier.plot_patch_clusters(patch_metadata, num_clusters=10)
